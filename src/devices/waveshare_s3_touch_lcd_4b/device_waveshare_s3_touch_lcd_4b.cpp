...
constexpr uint8_t kExpanderPanelReset = 7;
...
// Arduino_GFX 1.6.5's exact Waveshare ESP32-S3-Touch-LCD-4B profile uses 12 MHz with
...
constexpr const char* kPanelInitLabel = "type1-waveshare";
...
bool initDisplay() {
  ...
  g_panel_bus = new Arduino_XCA9554SWSPI(
      kExpanderPanelReset, kExpanderPanelCs, kExpanderPanelScl,
  ...
  Serial.printf(
      "[Device/Waveshare ESP32-S3-Touch-LCD-4B] Display ready, test=%s, "
...
bool DeviceWaveshareS3TouchLCD4B::initSDCard() {
  // The first bring-up intentionally leaves microSD disabled. The public
  // schematic audit does not yet provide a revision-qualified storage pin
  // mapping, and guessing here could collide with the shared I2C/RGB pins.
  return false;
#if 0
  ...
  if (!mounted) {
    g_sd_available = false;
    Serial.println(
        "[Device/Waveshare ESP32-S3-Touch-LCD-4B] SD card mount failed");
#if HOMETILES_GUITION_S3_DIAGNOSTICS_ACTIVE
    Serial.printf(
  ...
  if (SD.cardType() == CARD_NONE) {
    ...
    Serial.println("[Device/Waveshare ESP32-S3-Touch-LCD-4B] SD card absent");
#if HOMETILES_GUITION_S3_DIAGNOSTICS_ACTIVE
    Serial.printf(
  ...
  g_sd_available = true;
  Serial.printf(
      "[Device/Waveshare ESP32-S3-Touch-LCD-4B] SD card OK, size=%llu MB\n",
