 ...
 *
 * Modified for HomeTiles: the optional panel-ID readback was removed because
 * it is not required for initialization and can stall on ILI9881C DSI panels.
 */
...
static esp_err_t panel_ili9881c_init(esp_lcd_panel_t *panel)
{
    ...
    // Panel ID readback is intentionally skipped. It is informational only,
    // while DBI reads can block on panels that do not return these registers.
