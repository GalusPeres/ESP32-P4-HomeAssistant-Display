...
// I2C bus pins shared by helper controller (CH32V003 / TCA9554), GT911 touch, and RTC
...
// TCA9554 fallback registers (used on older Rev 1.0 - 3.0 boards)
...
constexpr uint8_t kTcaPinTouchRst = (1 << 0);
constexpr uint8_t kTcaPinBlEn = (1 << 1);
constexpr uint8_t kTcaPinLcdRst = (1 << 2);
...
bool initHelperController() {
  ...
  // Fallback: Probe TCA9554 expander (Rev 1.0 - 3.0)
  if (probeI2cAddress(kTcaAddress)) {
    Serial.println("[Device/Waveshare ESP32-S3-Touch-LCD-4] Detected TCA9554 expander (Rev 1.0-3.0)");
    g_helper_type = HelperType::TCA9554;

    // Outputs: EXIO0 (Touch RST), EXIO1 (BL EN), EXIO2 (LCD RST)
    const uint8_t out_mask = kTcaPinTouchRst | kTcaPinBlEn | kTcaPinLcdRst;
    writeI2cRegister(kTcaAddress, kTcaRegConfig, static_cast<uint8_t>(~out_mask));
    writeI2cRegister(kTcaAddress, kTcaRegOutput, 0x00);
    delay(200);
    writeI2cRegister(kTcaAddress, kTcaRegOutput, kTcaPinTouchRst | kTcaPinLcdRst);
    delay(200);
  ...
  Serial.println("[Device/Waveshare ESP32-S3-Touch-LCD-4] WARNING: Neither CH32V003 nor TCA9554 found on I2C bus");
  return false;
}
...
void applyBrightness(uint8_t value, bool remember = true) {
  ...
  } else if (g_helper_type == HelperType::TCA9554) {
    const uint8_t current_out = (value > 0) ? (kTcaPinTouchRst | kTcaPinLcdRst | kTcaPinBlEn)
                                            : (kTcaPinTouchRst | kTcaPinLcdRst);
    writeI2cRegister(kTcaAddress, kTcaRegOutput, current_out);
  }
