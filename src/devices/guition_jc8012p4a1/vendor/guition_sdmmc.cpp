...
bool ensure_sd_power() {
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/Guition JC8012P4A1] SD power control on LDO VO%d failed: %s (0x%x)\n",
                  kSdLdoChannel,
                  esp_err_to_name(err),
  ...
  Serial.printf("[Device/Guition JC8012P4A1] SD power control uses LDO VO%d\n",
                kSdLdoChannel);
  return true;
}
...
bool GuitionSDMMCFS::begin(const char* mountpoint,
  ...
  // Keep flags supplied by SDMMC_HOST_DEFAULT(), including DEINIT_ARG.
  ...
  if (err != ESP_OK) {
    Serial.printf("[Device/Guition JC8012P4A1] SD mount failed: %s (0x%x), freq=%d kHz\n",
                  esp_err_to_name(err),
                  static_cast<unsigned>(err),
                  sdmmc_frequency);
    card_ = nullptr;
